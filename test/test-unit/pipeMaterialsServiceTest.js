...
const {
    deletePipeMaterials,
    setPipeMaterials
} = require('../../api/dao/pipeMaterialsQueries')
...
describe('RECUPERACION DE GRUPO DE MAteriales de Caño - API REST', () => {
    ...
    before('Se crea listado de materiales para el testeo ', async () => {
        const result = await setPipeMaterials(genericRequest)
        genericId = result._id
        console.log(genericId)
    })

    it('[ GET - /app/pipe_materials ] Recupera todas los materiales para pintar la solapa', async () => {
        const responseClient = await fetch(req + 'pipe_materials', {
            method: 'GET',
            headers: {
    ...
    after('Se eliminan los registros testeados ', async () => {
        ...
        await deletePipeMaterials(genericId)
    })
})
